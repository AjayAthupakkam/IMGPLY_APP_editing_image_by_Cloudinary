        ...
        {imageUrl ? (
          ...
          >
            ...
            {(isTransforming || isImageLoading) && (
              <div className="absolute inset-0 bg-black/30 flex flex-col items-center justify-center">
                ...
                />
                <p className="text-white font-medium mt-2">Transforming your image...</p>
              </div>
            )}
            ...
            {isError && !isTransforming && (
              <div className="absolute inset-0 bg-black/10 flex items-center justify-center">
                <div className="error-container p-6 bg-red-50 text-red-600 rounded-xl border border-red-200 shadow-sm max-w-[80%]">
                  <p className="font-medium">Failed to load the transformed image.</p>
                  <p className="text-sm mt-2">Please try again with a more specific prompt.</p>
                  {type === 'remove' && (
                    <p className="mt-2 text-sm">For object removal, use specific terms like "person", "car", or "tree".</p>
                  )}
                </div>
              </div>
            )}
          ...
          >
            <div className="flex flex-col items-center justify-center p-6 text-center">
              ...
              <p className="text-gray-500 mb-1">{isTransformed ? 'Image loading...' : 'Transformed Image'}</p>
              <p className="text-gray-400 text-sm">Your image transformation will appear here</p>
            </div>
          </div>
        )}
      </div>
